...
**YOLO Detection 라벨링 툴**은 이미지에 바운딩 박스(Bounding Box)를 그려 YOLO 객체 탐지 모델의 학습 데이터를 만드는 도구입니다. 브라우저에서 바로 실행되며 별도 설치가 필요 없습니다.

**작업 결과물**

저장하면 아래 구조의 ZIP 파일이 생성됩니다.

```
annotations_2024-01-15.zip
├── json/          ← 작업 재개용 데이터
└── labels/        ← YOLO 학습용 TXT 파일
```
...
**https://sungeun-yoo.github.io/data_labeler/**
...
**Config 형식** — 라벨링할 클래스 이름을 나열한 JSON 배열 파일입니다.

```json
["person", "car", "truck", "bicycle", "motorcycle"]
```

- 배열의 순서가 YOLO 클래스 인덱스가 됩니다 (첫 번째 항목 = 0번).
- 로드에 성공하면 버튼이 초록색 **"Config 로드됨"** 으로 바뀝니다.

> [!WARNING]
> 형식이 잘못되면 오류가 표시됩니다. 반드시 `["클래스1", "클래스2"]` 형태의 JSON 배열이어야 합니다.
...
자동으로 이전 어노테이션을 복원합니다. 자세한 내용은 [9. 작업 이어하기](#9-작업-이어하기)를 참고하세요.
...
**좌표 직접 수정**: 오른쪽 패널 하단의 X1, Y1, X2, Y2 값을 직접 입력합니다.
...
ZIP의 `labels/` 폴더를 YOLO 학습에 바로 사용할 수 있습니다.
